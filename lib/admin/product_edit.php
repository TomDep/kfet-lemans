<?php
    ...
    if(isset($_FILES['file'])) {
        // Get the last image
        $data = select($mysqli, 'products', array('name', 'image'), $_POST['pk']);
        $filenameToRemove = $data[0]['image'];
        $filenameToSet = $data[0]['name'] . '.' . getFileExtansion($_FILES['file']['name']);

        // Remove it
        try {
            removeFile('../../res/images/products/', $filenameToRemove);
        } catch(Exception $e) {
            $warningMessage = $e->getMessage();
        }
        
        // Save the file to the directory
        if(!saveFile('../../res/images/products/', $_FILES['file'], $filenameToSet)) {
            $fileError = true;
        ...
        // Update the table
        update($mysqli, 'products', array(array('key' => 'image', 'value' => $filenameToSet)), $_POST['pk']);
    }

    // Update a value
    else if(isset($_POST['name'], $_POST['pk'], $_POST['value'])) {
        if(update($mysqli, 'products', array(array('key' => $_POST['name'], 'value' => $_POST['value'])), $_POST['pk'])) {
